...
public class StudentExamEntryEqualsTutorTest {
    ...
    public void testBasic() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            final int markIndex = random.nextInt(SolutionTableGenerator.VALID_MARKS.length);
            final int nextMarkIndex = (markIndex + 1) % SolutionTableGenerator.VALID_MARKS.length;
            final String mark = SolutionTableGenerator.VALID_MARKS[markIndex];
            final String nextMark = SolutionTableGenerator.VALID_MARKS[nextMarkIndex];
